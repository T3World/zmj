...
public class LoginServiceImpl implements LoginService {
    ...
    public SysResult doLogin(String userAccount, String userPassword) {
        ...
        try {
            ...
            List<ZZModuleEntity> rights = getModuleValuesByUserId(userId);
            List<ZZRoleEntity> roles = listRoleNamesByUserId(userId);
    ...
    private List<ZZRoleEntity> listRoleNamesByUserId(String userId){
        return roleUserMapper.listRoleNamesByUserId(userId);
    }

    private List<ZZModuleEntity> getModuleValuesByUserId(String userId){
        List<ZZModuleEntity> modules = moduleMapper.listModuleValueByUserId(userId);
        List<ZZModuleEntity> pModules = modules.stream().filter(m -> "0".equals(m.getmPid())).collect(Collectors.toList());
        for(ZZModuleEntity pm: pModules){
            List<ZZModuleEntity> collect = modules.stream().filter(e -> e.getmPid().equals(pm.getmId())).collect(Collectors.toList());
            pm.setChildren(collect);
        }
        return pModules;
    }
