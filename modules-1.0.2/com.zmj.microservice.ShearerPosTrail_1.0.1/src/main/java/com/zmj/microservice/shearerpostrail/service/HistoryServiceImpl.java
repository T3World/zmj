...
public  class HistoryServiceImpl implements HistoryService {
    ...
    @Autowired
    private ElectronicControlSystemContractService eleService;
...
//                    .multiply(new BigDecimal(-1))
    ...
    public SysResult<CommonVO> getInfraredPosWithAutoRunningData(BaseUNDTO fs){
        SysResult<CommonVO> autoResult = eleService.getIsAutoRunningData(fs);// 获取自动根机结果
        SysResult<CommonVO> infraredPos = eleService.getInfraredPosData(fs); // 获取煤机红外位置结果
        SysResult<CommonVO> result = new SysResult<>();
        List<CommonVO>  dataList = new ArrayList<>();
        if(autoResult.getErrcode()!=200){
            return  autoResult;
        }
        if(infraredPos.getErrcode()!=200){
            return  infraredPos;
        }
        logger.info(JSON.toJSONString(autoResult));
        logger.info(JSON.toJSONString(infraredPos));



        List<CommonVO>  autoData  =  autoResult.getData(); // 获取自动根机data值
        List<CommonVO> infposData =   infraredPos.getData(); // 获取煤机红外data值
        //去重
    ...
    public SysResult getInfraredPosWithAutoModeData(BaseUNDTO fs) {
        ...
        if (runningStateData.getErrcode()!=200 && runningStateData.getErrcode()!=204)
        ...
        List<CommonVO<Boolean>> list = runningStates.get(0).getData();//这里不知道会不会空指针!
    ...
    public SysResult<CommonVO> getPositionWithAutoRunningData(ShearerPositionDTO fs) throws IllegalParamException {
        SysResult<CommonVO> isAutoRun  =  eleService.getIsAutoRunningData(new BaseUNDTO(fs.getDataSourceName(),fs.getStartTime(),fs.getEndTime())); // 获取自动跟机的状态
        SysResult<CommonVO> position = this.getPositionData(fs); // 获取编码器位置
        List<CommonVO> autoList = isAutoRun.getData(); // 获取自动跟机状态数据
        List<CommonVO> posList = position.getData(); // 获取编码器位置数据
        SysResult result = new SysResult();
        List<CommonVO> list = new ArrayList<>();
        if(isAutoRun.getErrcode()!=200){
            return  isAutoRun;
        }
        if(position.getErrcode()!=200){
            return  position;
        }
        ...
        result.setErrcode(ResponseCodeEnum.OK.getValue());
        result.setErrmsg("访问成功");
        result.setData(comResult);
        return result;
    }


    @Override
    public SysResult getPositionWithAutoModeData(ShearerPositionDTO fs) throws ParseException, IllegalParamException {
        SysResult<CommonVO<Double>> positionData = this.getPositionData(fs);
        if (positionData.getErrcode()!=200) {
            return positionData;
        }
    ...
    public SysResult getTractionSpeedData(TractionSpeedDataDTO dto) {
        ...
        if (tractionSpeedData.getErrcode()!=200)
